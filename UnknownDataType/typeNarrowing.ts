...
function marks(howmuch : string | number) {
...
function serveFood(serve ?:string) {
...
function OrderPizza(PizzaSize:"Small" | "Regular" | "Large" | number) {
    if(PizzaSize==='Small') {
...
function serve(Pizza:FarmFresh | CheeseBurst) {
...
function checkType(check:any):check is PizzaType {
    return (
        typeof check==="object" && check!=null && typeof check.type==='string' && typeof check.price==='number' 
    ) 
...
function ServePizza(pizza:PizzaType | string) {
    if(checkType(pizza))
